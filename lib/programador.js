...
async function obtenerConfig() {
  const row = await (await db()).prepare('SELECT * FROM backup_config WHERE id = 1').get()
  const cfg = { ...row }
...
async function obtenerEmailConfig(incluirPassword) {
  const row = await (await db()).prepare('SELECT * FROM email_config WHERE id = 1').get()
  const cfg = { ...row }
  if (!incluirPassword) cfg.password = cfg.password ? '********' : ''
  return cfg
}

async function guardarEmailConfig(datos) {
  const b = datos || {}
  const actual = await (await db()).prepare('SELECT * FROM email_config WHERE id = 1').get()
...
async function enviarCopia(reg, opciones) {
  const o = opciones || {}
  const cfg = o.config || await obtenerConfig()
  const email = o.emailConfig || await obtenerEmailConfig(true)
  ...
  if (!cfg.correo_activo) {
    await (await db()).prepare(`INSERT INTO correo_envios (backup_id, destinatarios, asunto, tamano, checksum, estado, intentos, resumen, usuario)
  ...
  if (!destinatarios.length) {
    await (await db()).prepare(`INSERT INTO correo_envios (backup_id, destinatarios, asunto, tamano, checksum, estado, intentos, ultimo_error, resumen, usuario)
...
async function ejecutar(opciones) {
  ...
  const cfg = o.config || await obtenerConfig()
  const cnn = await db()
  const reg = await backups.generar({ db: cnn, tipo: o.tipo || 'automatico', usuario: o.usuario || 'sistema', config: cfg, notas: o.notas })
  ...
  if (correo.estado === 'fallido') {
    await alertarCorreoFallido(reg, correo.detalle)
  }
  return { backup: reg, correo }
}

async function alertarCorreoFallido(reg, error) {
  try {
    await (await db()).prepare(`INSERT INTO alertas_auditoria (tipo, severidad, descripcion, usuario, detalle)
...
async function historialEnvios(limite) {
  const cnn = await db()
  return await cnn.prepare('SELECT * FROM correo_envios ORDER BY id DESC LIMIT ?').all(Number(limite) || 100)
}
...
async function revisar(baseUrl) {
  ...
  if (!cfg.proximo_ejecutado) {
    ...
    await (await db()).prepare('UPDATE backup_config SET proximo_ejecutado = ? WHERE id = 1').run(proximo)
  ...
  try {
    ...
    const refresco = await obtenerConfig()
    await (await db()).prepare('UPDATE backup_config SET ultimo_ejecutado = ?, proximo_ejecutado = ? WHERE id = 1')
    ...
    await (await db()).prepare('UPDATE backup_config SET proximo_ejecutado = ? WHERE id = 1').run(reintento)
...
function iniciar(baseUrl) {
  ...
  Promise.resolve().then(async () => {
    ...
    if (cfg.activo && !cfg.proximo_ejecutado) {
      await (await db()).prepare('UPDATE backup_config SET proximo_ejecutado = ? WHERE id = 1').run(calcularProximo(cfg, new Date()))
    }
  }).catch(() => {})
