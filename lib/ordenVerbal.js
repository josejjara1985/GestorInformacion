...
function extraerCampos(row) {
  return {
    ...
    direccion_fiscal: texto(row.direccion_fiscal || [row.celular_fiscal, row.correo_fiscal].filter(Boolean).join('\n')),
    defensoria_min_publico: texto(row.defensoria_min_publico),
    direccion_min_publico: texto(row.direccion_min_publico || [row.celular_min_publico, row.correo_min_publico].filter(Boolean).join('\n')),
    victima: texto(row.victima),
    direccion_victima: texto(row.direccion_victima || [row.celular_victima, row.correo_victima].filter(Boolean).join('\n')),
    defensor: texto(row.defensor),
    direccion_defensor: texto(row.direccion_defensor || [row.celular_defensor, row.correo_defensor].filter(Boolean).join('\n')),
    direccion_detenido: texto(row.direccion_detenido),
    observacion: texto(row.observacion || row.observacion_orden_verbal)
  }
}
...
function cuerpoOficio(campos) {
  ...
  return `
    ...
    <p><strong>OBSERVACIÓN:</strong> ${nl(c.observacion)},</p>
