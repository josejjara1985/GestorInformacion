...
const PLANTILLA = path.join(__dirname, '..', 'data', 'plantillas', 'preparatoria.docx')
...
function reemplazarRunsAmarillos(xml, valores) {
  ...
  return xml.replace(reRun, (run) => {
    ...
    else if (DEJAR.has(original) || !original.includes('columna')) valor = original
    else return run
...
function camposPreparatoria(row) {
...
function encabezadoHtml() {
  return `<div class="ovc-header">
    <img class="ovc-logo" src="/api/oficios/preparatoria/media/image1.png" alt="Rama Judicial" />
...
function pieHtml() {
  return `<div class="ovc-footer">
    <img class="ovc-pie-logo" src="/api/oficios/preparatoria/media/image2.jpeg" alt="Certificaciones" />
...
function nombreArchivo(campos, id) {
  const base = ['preparatoria', campos.radicado_no || '', id]
    .filter(Boolean)
    .join('-')
    .replace(/[^\w.\-]+/g, '_')
    .slice(0, 80)
  return base || 'preparatoria'
}

function construirPreparatoria(row) {
  const campos = camposPreparatoria(row)
