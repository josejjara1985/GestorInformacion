...
function resolverDbPath() {
  const bruto = String(process.env.DB_PATH || '').trim()
  if (bruto) return path.resolve(ROOT, bruto)
...
function fsyncRuta(ruta) {
  ...
  try {
    ...
    // Windows puede devolver EPERM/EBADF/EINVAL sobre archivos en uso por SQLite.
    // El checkpoint(TRUNCATE) + synchronous=FULL ya garantizan persistencia.
    // No es motivo para abortar el guardado.
    if (e.code !== 'EPERM' && e.code !== 'EBADF' && e.code !== 'EINVAL') throw e
  } finally {
    if (fd !== undefined) {
...
function persistir(cnn) {
  ...
  try {
    ...
    try {
      ...
      return { ok: false, error: e2.message, ruta: DB_PATH }
    }
  }
    try {
    fsyncRuta(DB_PATH)
    ...
    try {
      ...
      try { fs.fsyncSync(dirFd) } catch (e) {
        if (e.code !== 'EPERM' && e.code !== 'EBADF' && e.code !== 'EINVAL') throw e
    ...
    return { ok: false, error: 'fsync: ' + e.message, ruta: DB_PATH, checkpoint: ultimo }
  }
  if (ultimo && Number(ultimo.busy) !== 0) {
    return { ok: false, error: 'checkpoint ocupado', ruta: DB_PATH, checkpoint: ultimo }
  }
