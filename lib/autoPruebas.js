...
const PLANTILLA = path.join(__dirname, '..', 'data', 'plantillas', 'auto-pruebas.docx')
...
function camposAutoPruebas(row) {
...
function encabezadoHtml() {
  return `<div class="ovc-header">
    <img class="ovc-logo" src="/api/oficios/auto-pruebas/media/image1.png" alt="Rama Judicial" />
...
function pieHtml() {
  return `<div class="ovc-footer">
    <img class="ovc-pie-logo" src="/api/oficios/auto-pruebas/media/image2.jpeg" alt="Certificaciones" />
...
function nombreArchivo(campos, id) {
  const base = ['auto-pruebas', campos.radicado_no || '', id]
    .filter(Boolean)
    .join('-')
    .replace(/[^\w.\-]+/g, '_')
    .slice(0, 80)
  return base || 'auto-pruebas'
}

function construirAutoPruebas(row) {
  const campos = camposAutoPruebas(row)
